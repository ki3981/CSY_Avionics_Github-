...
int LimitSwitch1 = 3;  // 리미트스위치1 핀 지정
int LimitSwitch2 = 5;  // 리미트스위치2 핀 지정
int Relay1 = 2;	// 낙하산 사출 릴레이모듈 핀 지정, 차후 수정 필요!!
unsigned long ejectionstartMillis = 0; // 낙하산 사출 릴레이모듈 열선 가열 시작 시간 지정
const unsigned long ejectioninterval = 10000; // 릴레이모듈 열선 가열 시간 10000ms로 설정
String cmd;
